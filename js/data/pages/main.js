window.ComfortSite = window.ComfortSite || {};
window.ComfortSite.pages = window.ComfortSite.pages || {};

window.ComfortSite.pages.main = {
  sec2Tabs: {
    pico: {
      imagePc: "./assets/images/main/removal_pc.png",
      imageMo: "./assets/images/main/remove_mo.png",
      image: "./assets/images/main/removal_pc.png",
      mainTitleParts: ["통증은 줄이고", "효과는 높이고"],
      serviceTitle: "피코 문신제거",
      serviceDesc: "피코레이저로 문신에 맞춰 효과적이고 세밀한 시술이 가능합니다."
    },
    semi: {
      imagePc: "./assets/images/main/removal02_pc.png",
      imageMo: "./assets/images/main/remove02_mo.png",
      image: "./assets/images/main/removal02_pc.png",
      mainTitleParts: ["잔흔은 줄이고", "자연스러움은 높이고"],
      serviceTitle: "반영구 문신제거",
      serviceDesc: "예민한 부위인 만큼 섬세한 조사로 부작용을 줄이며 효과를 높입니다."
    },
    scalp: {
      imagePc: "./assets/images/main/removal03_pc.png",
      imageMo: "./assets/images/main/remove03_mo.png",
      image: "./assets/images/main/removal03_pc.png",
      mainTitleParts: ["두피 상태에 맞춰", "안전하게 진행하는 문신제거"],
      serviceTitle: "두피 문신제거",
      serviceDescHtmlDesktop: "잘못되었거나 번진 두피 문신도 두피와 머리카락의 손상을 최소화하여 시술합니다.",
      serviceDescHtmlMobile: "잘못되었거나 번진 두피 문신도<br />두피와 머리카락의 손상을 최소화하여 시술합니다."
    },
    surgical: {
      imagePc: "./assets/images/main/removal04_pc.png",
      imageMo: "./assets/images/main/remove04_mo.png",
      image: "./assets/images/main/removal04_pc.png",
      mainTitleParts: ["부위 특성에 맞춘", "수술적 문신제거"],
      serviceTitle: "수술적 문신제거",
      serviceDesc: "면접이나 결혼 등으로 빠른 시일 내 제거를 해야 하는 분들께 추천드립니다."
    }
  },
  sec7FallbackResults: [
    {
      round: "[손가락] 5회차",
      caption: "시술 후 4개월 경과",
      beforeImageUrl: "",
      beforePosition: "center",
      afterImageUrl: "",
      afterPosition: "center"
    },
    {
      round: "[팔] 3회차",
      caption: "시술 후 2개월 경과",
      beforeImageUrl: "",
      beforePosition: "center",
      afterImageUrl: "",
      afterPosition: "center"
    },
    {
      round: "[눈썹] 2회차",
      caption: "시술 후 1개월 경과",
      beforeImageUrl: "",
      beforePosition: "center",
      afterImageUrl: "",
      afterPosition: "center"
    },
    {
      round: "[반영구] 4회차",
      caption: "시술 후 3개월 경과",
      beforeImageUrl: "",
      beforePosition: "center",
      afterImageUrl: "",
      afterPosition: "center"
    }
  ]
};
